...
os_check()
{
  ...
  echo "You need to be running Ubuntu 14.04"
    exit
  fi
}
...
echo "Welcome to Venison for Ubuntu 14.04"
echo ""
...
install_base()
{
  ...
  update-rc.d -f xinetd remove > /dev/null 2>&1
  echo "done."
}
...
install_php()
{
  ...
  mkdir -p /var/www
  aptitude -y install php5-cli php5-common php5-mysql php5-gd php5-curl > /dev/null 2>&1
  aptitude -y install php5-fpm php5-cgi php-pear php5-dev libpcre3-dev > /dev/null 2>&1
...
install_mysql()
{
  ...
  echo "percona-server-server-5.5 percona-server-server/root_password password $MYSQL_PASS" | debconf-set-selections
  echo "percona-server-server-5.5 percona-server-server/root_password_again password $MYSQL_PASS" | debconf-set-selections
  aptitude -y install percona-xtradb-cluster-server-5.5 > /dev/null 2>&1
